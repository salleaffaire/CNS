...
namespace tui {
// Event handling
enum EventType { KEYDOWN, MOUSEBUTTONDOWN, UNDEFINED };
...
bool operator==(const Paragraph& paragraph1, const Paragraph& paragraph2) {
  return (
      ...
      paragraph1.text == paragraph2.text && paragraph1.x == paragraph2.x &&
      paragraph1.y == paragraph2.y && paragraph1.width == paragraph2.width &&
      paragraph1.height == paragraph2.height &&
      paragraph1.border == paragraph2.border &&
      paragraph1.border_style.foreground ==
          paragraph2.border_style.foreground &&
      paragraph1.border_style.background ==
          paragraph2.border_style.background &&
...
bool operator==(const List& list1, const List& list2) {
  return (list1.title == list2.title && list1.rows == list2.rows &&
          list1.x == list2.x && list1.y == list2.y &&
          list1.width == list2.width && list1.height == list2.height &&
...
bool operator==(const BarChart& bar_chart1, const BarChart& bar_chart2) {
  return (
      ...
      bar_chart1.x == bar_chart2.x && bar_chart1.y == bar_chart2.y &&
      ...
      bar_chart1.number_style.background ==
          bar_chart2.number_style.background &&
      ...
      bar_chart1.border_style.foreground ==
          bar_chart2.border_style.foreground &&
      bar_chart1.border_style.background ==
          bar_chart2.border_style.background &&
      bar_chart1.text_style.foreground == bar_chart2.text_style.foreground &&
      bar_chart1.text_style.background == bar_chart2.text_style.background &&
...
bool operator==(const Gauge& gauge1, const Gauge& gauge2) {
  return (gauge1.title == gauge2.title &&
          gauge1.bar_color == gauge2.bar_color && gauge1.x == gauge2.x &&
          gauge1.y == gauge2.y && gauge1.width == gauge2.width &&
          gauge1.height == gauge2.height && gauge1.border == gauge2.border &&
          gauge1.percent == gauge2.percent && gauge1.label == gauge2.label &&
...
class Window {
 public:
  ...
  void draw_title(Widget widget) {
    short title_color =
        get_color(widget.title_style.foreground, widget.title_style.background);
    for (int i = widget.x + 2;
         i <
         widget.x + std::min(widget.width, (int)(widget.title.length()) + 2);
         i++) {
...
#ifdef IS_WIN
  Window(int window_width_ = 0, int window_height_ = 0)
      : window_width((LONG)window_width_), window_height((LONG)window_height_) {
  ...
  void remove_scrollbar() {
    ...
    COORD new_size = {(short)(csbi.srWindow.Right - csbi.srWindow.Left + 1),
  ...
  inline CHAR_INFO* get_content() { return content; }
#elif defined(IS_POSIX)
  Window() {
    initscr();
    raw();
  ...
  // Hide cursor from console
  inline void hide_cursor() { curs_set(0); }

  // Show cursor from console
  inline void show_cursor() { curs_set(1); }
...
void Window::add(Paragraph paragraph) {
  ...
  // Get color
  short text_color = get_color(paragraph.text_style.foreground,
  ...
  for (int i = paragraph.y + 1; i < paragraph.y + paragraph.height - 1; i++) {
    ...
    for (int j = paragraph.x + 1; j < paragraph.x + paragraph.width - 1; j++) {
      ...
      int current_index =
          (current_row * (paragraph.width - 2)) + current_column;
...
void Window::add(List list) {
  ...
  // Get color
  short text_color =
      get_color(list.text_style.foreground, list.text_style.background);
  for (int i = list.y + 1; i < list.y + list.height - 1; i++) {
    ...
    for (int j = list.x + 1; j < list.x + list.width - 1; j++) {
      ...
      draw_char(j, i, ' ');
      if (current_row < list.rows.size() &&
          current_column < list.rows[current_row].length()) {
        if (list.rows[current_row].length() > list.width - 2 &&
            j >= (list.x + list.width - 4)) {
          // Draw ellipsis
          if (list.width - 2 >= 3) {
...
void Window::add(BarChart bar_chart) {
  ...
  // Get colors
  short label_color = get_color(bar_chart.label_style.foreground,
                                bar_chart.label_style.background);
  short number_color = get_color(bar_chart.number_style.foreground,
  ...
  for (int i = bar_chart.x + 1; i < bar_chart.x + bar_chart.width - 1;
       i += bar_chart.bar_width + 1) {
    if (current_bar < bar_chart.labels.size()) {
      for (int j = 0; j < bar_chart.bar_width; j++) {
        if (j < bar_chart.labels[current_bar].size() &&
            i + j < bar_chart.x + bar_chart.width - 1) {
          draw_char((i + j), (bar_chart.y + bar_chart.height - 2),
                    bar_chart.labels[current_bar][j], label_color);
        }
      }
    }
    if (current_bar < bar_chart.data.size()) {
      auto draw_numbers = [&]() {
        for (int j = 0; j < bar_chart.bar_width; j++) {
          std::string current_number =
              std::to_string(bar_chart.data[current_bar]);
          if (j < current_number.length() &&
              i + j < bar_chart.x + bar_chart.width - 1) {
            draw_char((i + j), (bar_chart.y + bar_chart.height - 3),
                      current_number[j], number_color);
          }
        }
      };
      ...
      for (int y = bar_chart.y + maximum_height - height + 2;
           y < bar_chart.y + maximum_height + 1; y++) {
        for (int x = 0; x < bar_chart.bar_width; x++) {
#ifdef IS_WIN
          // Skip draw char to avoid overriding char
          content[y * columns_ + (i + x)].Attributes =
...
void Window::add(Gauge gauge) {
  ...
  // Get color
  short label_color =
      get_color(gauge.label_style.foreground, gauge.label_style.background);
  // Draw bar
...
#ifdef IS_WIN
        content[(i * columns_) + j].Attributes =
  ...
  for (int i = gauge.x + 1; i < gauge.x + gauge.width - 1; i++) {
    if (current_char < gauge.label.length()) {
      ...
      draw_char(i, label_y, gauge.label[current_char], color);
    }
    current_char++;
  }
}
...
void List::scroll_down(Window& window, int factor) {
  if (rows.size() > height - 2) {
    first_element =
        std::min((int)(rows.size() - (height - 2)), (first_element + factor));
    window.add(*this);
  }
}
};  // namespace tui
